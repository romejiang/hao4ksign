...
async function play() {
  ...
  try {
    ...
    const loginURL = 'https://www.javbus.com/forum/forum.php?mod=forumdisplay&fid=2'
    ...
    // 检查两个页面状态，已判断显示是否签到
    let checklist = [page.waitForSelector('.sign-button'), 
    page.waitForSelector('#seccodeverify_cSSPVVbJ')]
    ...
    if (champion) {
      ...
      // 检查状态，根据不同的状态打印日志
      if (className.includes('sign-button')) {
